...
function EstablishmentForm({ onEstabCreation }) {
  ...
  return (
    <>
    <div className="">
      <h2 className='m-3 text-center text-lime-700'>Create a new establishment</h2>
      <form onSubmit={handleSubmit(onEstablishmentSubmit)}>
        ...
        <div className="relative  w-full">
          <div className="absolute flex-1 w-full inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6  text-gray-500 dark:text-gray-400">
          ...
          <input type='text' placeholder='Name' className=" w-full bg-gray-50 border border-lime-300 text-gray-900 text-sm rounded-l-lg focus:ring-lime-500 focus:border-lime-500 block pl-12 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
          {...register('name', { required: 'Esablishment name is required'
          })} />
          
        </div>
        <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-lime-500 border border-lime-300 rounded-r-lg hover:bg-lime-600 hover:text-white focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-blue-500 dark:focus:text-white">
        ...
        {errors.name && <p class="m-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">Oops! </span>{errors.name?.message}</p>}
      </form>
      </div>
    </>
  )
